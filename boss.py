...
class Boss(pygame.sprite.Sprite):
    def __init__(self, x, y, trajectory):
        ...
        self.attack_hitbox = pygame.Rect(0, 0, 100, 40)
    ...
    def update_invincibility(self):
        """Обновление таймеров неуязвимости"""
        if self.invincible:
    ...
    def check_player_attack(self, player):
        ...
        if not player.attack or self.invincible:
            return
            
        self.attack_hitbox.midtop = self.hitbox.midtop
        self.attack_hitbox.y += 5 
        
        player_hit = pygame.Rect(
            player.rect.centerx - 15,
            player.rect.bottom - 10,
            30, 10
        )
        
        if player_hit.colliderect(self.attack_hitbox):
            self.sounds.play_sound('attack')
            self.take_damage()
